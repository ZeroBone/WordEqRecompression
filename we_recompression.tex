...
\begin{frame}
	...
	\begin{itemize}
		...
		\item This is because if all occurrences of $ ab $ are implicit, the solution $ \varphi $ is not length-minimal as we can get a shorter solution by defining $ \varphi' $ with $ \varphi'(\var{x}) $ set to $ \varphi(\var{x}) $ with every $ ab $ replaced by $ \varepsilon $ (see \cite[Lemma 2.4]{jez:2016} and \cite[Lemma 6]{plandowsky_rytter:1998}).
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item The latter implies that $ \varphi $ is not length-minimal because all occurrences of maximal $ d^k $ blocks can be deleted from the solution. This is a contradiction.
		\item Hence,
	\end{itemize}
...
\begin{frame}
	\begin{lemma}[Restated, cf. \cite{jez:2016, plandowsky_rytter:1998}]
		Let $ \varphi $ be a length-minimal solution to $ U = V $. If $ a^k \neq \varepsilon $ is a maximal block in $ \varphi(U) $, then there exists an \textit{explicit} or \textit{crossing} occurrence of a maximal block $ a^k $ in $ \varphi(U) $, i.e., an occurrence where at least one $ a $ of $ a^k $ corresponds to an $ a $ appearing in either $ U $ or $ V $.
	\end{lemma}
	\begin{proof}
		Suppose all occurrences of the maximal block $ a^k $ were \textit{implicit}, i.e., were substrings of $ \varphi(\var{x}) $ for some $ \var{x} \in \X $. For each such occurrence, there w.l.o.g. exist $ b, c \in \Gamma $ such that $ ba^kc $ is a substring of $ \varphi(U) $ for some $ b \neq a \neq c $; replace every such occurrence 
	\end{proof}
\end{frame}
